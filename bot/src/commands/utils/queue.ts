...
export class QueueButtonCommand implements buttonCommand {
    ...
    public async execute(interaction: ButtonInteraction, player: Player): Promise<void> {
        ...
        if (!member.voice.channel) return void await interaction.followUp({ content: "You are not in a voice channel.", flags: "Ephemeral" });

        const botVoiceChannelId = interaction.guild?.members.me?.voice.channelId;
        if (botVoiceChannelId && member.voice.channelId !== botVoiceChannelId) return void await interaction.followUp({ content: "You are not in my voice channel.", flags: "Ephemeral" });
        ...
        const buildButtons = (pageIndex: number, collectorActive: boolean = true): ActionRowBuilder<ButtonBuilder> => new ActionRowBuilder<ButtonBuilder>().addComponents(
            new ButtonBuilder()
        ...
        const msg = await interaction.followUp({ embeds: [buildEmbed(page)], components: [buildButtons(page)] }) as Message;
        const collector = msg.createMessageComponentCollector({ time: 60_000, filter: i => i.user.id === interaction.user.id });

        collector.on("collect", async (btnInt) => {
            if (!btnInt.deferred && !btnInt.replied) await btnInt.deferUpdate();
        ...
        collector.on("end", async () => {
            try {
                ...
                if (fetchedMsg) await fetchedMsg.edit({ components: [buildButtons(page, false) ]});
            } catch (error) {
