...
export class ButtonHandler {
    ...
    private loadCommands() {
        const buttonCommands: buttonCommand[] = [
    ...
    public async handle(interaction: ButtonInteraction): Promise<void> {
        ...
        if (command) {
            try {
                ...
                if (!interaction.deferred && !interaction.replied) await interaction.reply({ content: "An error occurred while executing command.", flags: "Ephemeral" });
                else await interaction.followUp({ content: "An unexpected error occurred.", flags: "Ephemeral" });
            }
        } else {
